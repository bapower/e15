...
The node executable is simple to use. Although it has other arguments, usually you will pass only one argument, the name of your main Node.js source file. For example: 
```
node hello.js
```
...
To install a package, first find the npm package that you want to install. Packages can be found on Github, [the npm website](https://www.npmjs.com/), or using the search command in the npm executable itself. 
...
Node.js is composed mostly of nonblocking APIs. Each statement is still executed in order, but a function passed as an argument to the call is invoked when the statement is completely done. The next statement in order might execute before or after that happens. 
...
* PHP, especially using a framework such as laravel, is easier to learn and quicker to get up in running
